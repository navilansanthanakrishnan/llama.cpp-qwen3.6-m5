...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv_sgq4k(ggml_metal_library_t lib, const ggml_tensor * op, int nsg) {
    char base[256];
    char name[256];
    ...
    snprintf(base, 256, "kernel_mul_mv_sgq%dk_f32",
             op->src[0]->type == GGML_TYPE_Q6_K ? 6 :
             op->src[0]->type == GGML_TYPE_Q5_K ? 5 : 4);
